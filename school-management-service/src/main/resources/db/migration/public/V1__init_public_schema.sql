...
CREATE UNIQUE INDEX uq_school_active_subscription ON school_subscriptions(school_id)
WHERE
    status = 'ACTIVE';
...
CREATE TABLE subscription_payments(
    ...
    CONSTRAINT chk_sub_payment_method CHECK (payment_method IN ('BANK_TRANSFER', 'ORANGE_MONEY', 'MTN_MONEY', 'WAVE', 'CARD', 'CHECK')),
    CONSTRAINT chk_sub_payment_amount CHECK (amount > 0),
    CONSTRAINT chk_sub_payment_period CHECK (period_start IS NULL OR period_end IS NULL OR period_end > period_start)
);
...
CREATE INDEX idx_school_memberships_person ON school_memberships(person_id)
WHERE
    is_active = TRUE;
